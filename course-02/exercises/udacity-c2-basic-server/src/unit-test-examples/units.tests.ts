...
describe('concat', ()=> {
  it('should concatenate mar and celo', () => {
    expect(concat("mar","celo")).to.equal("marcelo");
  });

  it('should throw error if parameter empty', ()=> {
    expect(() => { concat("mar","") }).to.throw("parameters must not be empty");
  });
});